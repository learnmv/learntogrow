-- Migration: Remove applet_config from geogebra_diagram prompt
-- The frontend now hardcodes width, height, and all UI-hiding flags in
-- the GeoGebraApplet constructor.  Telling the LLM to generate these
-- values only creates noise and occasional bad data.

UPDATE question_prompts
SET content = 'Generate a {question_type} math question for Grade {grade_level} that INCLUDES an interactive GeoGebra diagram.
