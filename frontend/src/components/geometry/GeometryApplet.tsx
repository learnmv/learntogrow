...
export function GeometryApplet({
  commands = [],
  config = {},
  height = 400,
  width = 600,
  ...
  const containerIdRef = useRef<string>(`ggb-geometry-${Math.random().toString(36).substring(2, 9)}`)
  ...
  useEffect(() => {
    ...
    async function initApplet() {
      try {
        ...
        // Default parameters for geometry applet
        ...
        timeoutId = setTimeout(() => {
          ...
          if (isMounted && !appletApi) {
            setError('Geometry applet failed to initialize. Please refresh the page.')
            setIsLoading(false)
          }
        }, 15000)
      } catch (err) {
        ...
        if (isMounted) {
          setError('Failed to load geometry applet. Please check your internet connection.')
          setIsLoading(false)
        }
      }
    }
      ...
      {isLoading && (
        ...
        >
          <div className="text-center">
            ...
            <p className="text-text-muted font-display text-sm">Loading geometry tool...</p>
          </div>
        </div>
      )}
      <div
