...
export function ClassicApplet({
  ...
  const containerIdRef = useRef<string>(`ggb-classic-applet-${Math.random().toString(36).substring(2, 9)}`)
  ...
  useEffect(() => {
    ...
    async function initApplet() {
      try {
        ...
        // Default parameters for classic applet - includes all features: graphing, geometry, 3D, CAS
        ...
        timeoutId = setTimeout(() => {
          ...
          if (isMounted && !appletApi) {
            setError('Classic applet failed to initialize. Please refresh the page.')
            setIsLoading(false)
          }
        }, 15000)
      } catch (err) {
        ...
        if (isMounted) {
          setError('Failed to load classic applet. Please check your internet connection.')
          setIsLoading(false)
        }
      }
    }
      ...
      {isLoading && (
        ...
        >
          <div className="text-center">
            ...
            <p className="text-text-muted font-display text-sm">Loading classic calculator...</p>
          </div>
        </div>
      )}
      <div
