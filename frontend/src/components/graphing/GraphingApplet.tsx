...
interface GraphingAppletProps {
  ...
  width?: number
  /** Applet type - determines which GeoGebra app to use */
  appletType?: 'graphing' | 'geometry' | '3d' | 'classic' | 'cas' | 'scientific'
  /** Callback when commands are executed, returns array of results */
...
export function GraphingApplet({
  commands = [],
  config = {},
  height = 400,
  width = 600,
    ...
    async function initApplet() {
      try {
        ...
        const parameters: Record<string, unknown> = {
          ...
          showAxes: true,
          showGrid: true,
          // This callback is called when the applet signals it's ready
          appletOnLoad: () => {
            if (!isMounted) return
            // Applet has signaled it's ready, but we still need to get the API
          },
          ...config,
        }
        ...
        checkReadyInterval = setInterval(() => {
          ...
          try {
            ...
            if (api && typeof api.evalCommand === 'function') {
              ...
              // Filter out commands that have already been executed
              const newCommands = commands.filter(cmd => !executedCommandsRef.current.has(cmd))

              if (newCommands.length > 0) {
                const results: { command: string; success: boolean }[] = []
                newCommands.forEach((command) => {
                  try {
                    const success = api.evalCommand(command)
                    results.push({ command, success: !!success })
                    // Mark as executed
                    executedCommandsRef.current.add(command)
                  } catch (err) {
                    console.warn('Failed to execute GeoGebra command:', command, err)
                    results.push({ command, success: false })
                    executedCommandsRef.current.add(command)
                  }
                })
                // Report results back to parent component
        ...
        timeoutId = setTimeout(() => {
          ...
          if (isMounted && !appletApi) {
            setError('Graphing applet failed to initialize. Please refresh the page.')
            setIsLoading(false)
          }
        }, 15000)
      } catch (err) {
        ...
        if (isMounted) {
          setError('Failed to load graphing applet. Please check your internet connection.')
          setIsLoading(false)
        }
      }
    }
      ...
      {isLoading && (
        ...
        >
          <div className="text-center">
            ...
            <p className="text-text-muted font-display text-sm">Loading graphing tool...</p>
          </div>
        </div>
      )}
      <div
